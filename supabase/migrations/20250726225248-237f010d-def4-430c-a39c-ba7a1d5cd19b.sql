...
CREATE OR REPLACE FUNCTION get_template_item_quantity_for_date(
  template_item_row customer_template_items%ROWTYPE,
  target_date DATE
)
RETURNS NUMERIC AS $$
DECLARE
...
BEGIN
  ...
  CASE day_of_week
    WHEN 0 THEN RETURN template_item_row.sunday_qty;
    WHEN 1 THEN RETURN template_item_row.monday_qty;
    WHEN 2 THEN RETURN template_item_row.tuesday_qty;
    WHEN 3 THEN RETURN template_item_row.wednesday_qty;
    WHEN 4 THEN RETURN template_item_row.thursday_qty;
    WHEN 5 THEN RETURN template_item_row.friday_qty;
    WHEN 6 THEN RETURN template_item_row.saturday_qty;
    ELSE RETURN 0;
  END CASE;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      qty_for_day := get_template_item_quantity_for_date(template_item_rec, p_date);
