...
ALTER TABLE public.customer_template_items ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for customer_templates
CREATE POLICY customer_templates_org_access 
  ON public.customer_templates 
  FOR ALL 
  TO authenticated 
  USING (organization_id = get_user_organization_id(auth.uid()));

-- Create RLS policies for customer_template_items
CREATE POLICY customer_template_items_org_access 
  ON public.customer_template_items 
  FOR ALL 
  TO authenticated 
  USING (organization_id = get_user_organization_id(auth.uid()));
