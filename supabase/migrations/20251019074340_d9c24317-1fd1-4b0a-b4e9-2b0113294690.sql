-- Drop the 2-parameter version and create the 3-parameter version
DROP FUNCTION IF EXISTS create_invoice_from_sales_order_sql(UUID, UUID);
...
COMMENT ON FUNCTION create_invoice_from_sales_order_sql IS 'Creates an invoice from a sales order with all line items and updates the order status';