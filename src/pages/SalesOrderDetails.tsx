    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8 pb-20 md:pb-8">
      ...
      {duplicateWarning && !['invoiced', 'cancelled'].includes(order.status) && (
        <Alert className="border-yellow-300 bg-yellow-50 text-yellow-800">
          ...
          <AlertDescription>
            ⚠️ This customer has another order for this date: <strong>{duplicateWarning.invoice_number}</strong> (Status: {duplicateWarning.status}, Total: ${duplicateWarning.total?.toFixed(2) || '0.00'})
          </AlertDescription>
        </Alert>
      )}
